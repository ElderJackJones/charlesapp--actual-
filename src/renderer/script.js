...
const getNetwork = async () => {
  ...
  if (strength < -90) {
    Alpine.store('alerts').push({
      color: 'alert-danger',
      content: '😱 Your internet is pretty slow... Be patient or wait for WIFI',
    });
  } else {
    Alpine.store('alerts').push({
      color: 'alert-success',
...
const getReport = async () => {
  ...
  Alpine.store('alerts').push({
    color: 'alert-info',
    content: "Don't close the window, this'll take a while... 👉👈",
  });
  let reportThings;
  try {
...
window.api.httpError(data => {
  Alpine.store('alerts').push({
    color: 'alert-danger',
    content:
      '💀 Something Critical Died. Try closing, waiting 5 minutes, then trying again.',
  });
});
...
const login = async (username, userpassword, botmail, botpass) => {
  ...
  if (worked) {
    Alpine.store('alerts').push({
      color: 'alert-success',
      content: 'Charles has your identity 🥸',
    });
  } else {
...
const sendZones = async (zones, $data) => {
  ...
  if (success) {
    Alpine.store('alerts').push({
      color: 'alert-success',
      content: 'Charles is looking over your little ID eggs',
    });
  } else {
    Alpine.store('alerts').push({
      color: 'alert-danger',
      content: '😱 #askedForAFish--gotASerpent',
    });
  }
};
...
const addToClipboard = async (arrayOfStrings, search) => {
  console.log(search);
  let message = '';
  for (const area of arrayOfStrings) {
    if (search) {
