...
const WINDOW_API = {
  ...
  personUpdate: callback =>
    ipcRenderer.on('person/update', (_event, data) => callback(data)),
  personBegin: callback =>
    ipcRenderer.on('person/begin', (_event, data) => callback(data)),
  personComplete: callback =>
    ipcRenderer.on('person/complete', (_event, data) => callback(data)),
  messageBegin: callback =>
    ipcRenderer.on('message/begin', (_event, data) => callback(data)),
  messageSent: callback =>
    ipcRenderer.on('message/sent', (_event, data) => callback(data)),
  httpError: callback =>
    ipcRenderer.on('httpError', (event, data) => callback(data)),
  messageComplete: callback =>
